...
class DataSet(torch.utils.data.Dataset):
    ...
    def __init__(self, voxels, hard_element_stiffness, hard_element_force, soft_element_stiffness, soft_element_force, C_hard, C_soft, jacobi, propertys, voxel_length, case, device):
        self.voxels = torch.tensor(voxels).float().to(device)
        self.hard_element_stiffness = torch.tensor(
            hard_element_stiffness).float().to(device)
        self.hard_element_force = torch.tensor(
            hard_element_force).float().to(device)
        self.soft_element_stiffness = torch.tensor(
            soft_element_stiffness).float().to(device)
        self.soft_element_force = torch.tensor(
            soft_element_force).float().to(device)
        ...
        self.propertys = torch.tensor(propertys).float().to(device)
    ...
    def __getitem__(self, index):
        ...
        return input, voxel, hard_stiffness, hard_force, soft_stiffness, soft_force, c_hard, c_soft, jacobi_, property
...
def load_dataset(base_path, voxel_length, case, train_ratio, device):
    ...
    voxel=np.load(os.path.join(base_path,'voxel.npz'))
    ...
    jacobi = data['J']
    propertys = data['CH']
    dataset = DataSet(voxels,  hard_element_stiffness_train, hard_element_force_train, soft_element_stiffness_train,
                      soft_element_force_train, C_hard, C_soft, jacobi, propertys, voxel_length, case, device)
