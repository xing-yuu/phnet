...
import config
from train.dataset_with_property import *
from train.eval_with_property import *
...
def dh_net(my_config):
    ...
    train_dataset, _ = load_dataset(
        dataset_dir, r, sample_per_voxel, train_ratio, device)
    ...
    tbdir = os.path.join(
        out_dir, 'log', datetime.datetime.now().strftime("%m-%d@%H-%M-%S"))
    writer = SummaryWriter(tbdir)

    for e in tqdm(range(epochs)):
        iter = tqdm(train_loader)
        for step, (input, voxels, hard_element_stiffness, hard_element_force, soft_element_stiffness, soft_element_force, _, _, _, property) in enumerate(iter):

            output = net(input)
            tic = time.perf_counter()
            loss = loss_func(output, net_homo, voxels, hard_element_stiffness,
                             hard_element_force, soft_element_stiffness, soft_element_force)
            ...
            tqdm_str = '[GPU {:<2d}] Epoch ={:<2d} | loss={:+.6f}'.format(
                0, e, loss.item())
            writer.add_scalar('Loss', loss.item(), step+e*len(train_loader))

            if step > 0 and step % eval_interval == 0:
                batch_error_mean, batch_error = eval(
                    net_homo, property, voxels, output, hard_element_stiffness, hard_element_force, ke, X0)
                tqdm_str += ', mean error={:+.6f}'.format(batch_error_mean)

                writer.add_scalar('Error_Avg', batch_error_mean,
                                  step+e*len(train_loader))
                for i in range(len(batch_error)):
                    scalar_name = 'Error' + str(i)
                    writer.add_scalar(
                        scalar_name, batch_error[i], step+e*len(train_loader))
